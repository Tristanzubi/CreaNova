...
### Front-end
✅ **React Hooks** : `useState`, `useParams`, hook personnalisé `useAuth`
✅ **TypeScript** : Types stricts pour File, ChangeEvent, FormEvent
✅ **Validation côté client** : Taille fichier, filtrage de tags vides
✅ **Upload fichier** : Input file, prévisualisation, FormData
✅ **UX/UI** : Prévisualisation image, toasts de feedback, protection de route

### Back-end
✅ **Architecture MVC** : Séparation Controller / Repository
✅ **Relations many-to-many** : Table de jointure `artwork_category`
✅ **Logique conditionnelle complexe** : Insertion conditionnelle de catégories
✅ **Traitement de données** : Filtrage, déduplication, split/join de chaînes
✅ **Transactions multiples** : Insertion artwork + catégories + relations
...
✅ Upload d'image avec validation (500ko max) et prévisualisation
✅ Formulaire complet (titre, description, prix, catégorie, 3 tags)
✅ Protection de route (artistes connectés uniquement)
✅ Gestion intelligente des catégories (création si inexistante)
✅ Déduplication automatique des tags
✅ Relations many-to-many (une œuvre → plusieurs catégories)
✅ Feedback utilisateur avec toasts

### Complexité technique
**Logique métier avancée :**
- Insertion conditionnelle (SELECT puis INSERT si nécessaire)
- Gestion de relations many-to-many
- Transactions multiples coordonnées
- Traitement et nettoyage de données

**Stack technique maîtrisée :**
- **Front-end :** React 19 + TypeScript + React Router v7 + FormData API
- **Back-end :** Express + TypeScript + MySQL2 (requêtes multiples)
- **Upload :** Multer pour gestion de fichiers

---

## 💭 RESSENTI PERSONNEL

Cette fonctionnalité a été l'une des plus enrichissantes à développer dans le projet. Au départ, la gestion des catégories et tags semblait simple, mais j'ai rapidement réalisé la complexité des relations many-to-many et de l'insertion conditionnelle.

**Défis rencontrés :**
Le plus grand défi a été de concevoir une logique robuste pour éviter les doublons de catégories. J'ai dû réfléchir à la séquence des requêtes SQL : vérifier l'existence, insérer si nécessaire, récupérer l'ID, puis créer la relation. Gérer cette logique dans une boucle pour les tags a nécessité une attention particulière pour éviter les erreurs de synchronisation.

**Apprentissages clés :**
Cette réalisation m'a permis de maîtriser plusieurs concepts avancés :
- Les **relations many-to-many** et leur gestion avec une table de jointure
- L'**insertion conditionnelle** en SQL (SELECT avant INSERT)
- Le **traitement de données** côté serveur (filtrage, déduplication avec `Set`)
- La **coordination de transactions multiples** dans un même flux

**Satisfaction :**
Je suis particulièrement satisfait de la fluidité de l'expérience utilisateur : la prévisualisation d'image instantanée, la flexibilité des tags optionnels, et le feedback immédiat via les toasts. Côté back-end, la logique d'insertion conditionnelle rend le système évolutif et autonome, ce qui est une vraie plus-value pour la maintenance future.

Cette fonctionnalité démontre ma capacité à gérer des flux de données complexes et à concevoir des architectures de base de données relationnelles efficaces.

---

**Réalisation :** Ajout d'œuvre avec gestion de catégories et tags
**Lignes de code :** ~125 (front) + ~70 (back)
**Fichiers :** 3 (AddArtworkPage.tsx, artworkActions.ts, artworkRepository.ts)
**Tables impactées :** 3 (artwork, category, artwork_category)
